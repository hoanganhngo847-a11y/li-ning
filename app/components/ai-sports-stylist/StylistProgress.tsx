...
export default function StylistProgress({ currentStep, onSelectStep }: StylistProgressProps) {
  ...
  return (
    <div className="flex items-center justify-center gap-2.5 sm:gap-4 md:gap-6 lg:gap-8 overflow-x-auto no-scrollbar py-1">
      {STYLIST_STEPS.map((step, idx) => {
        const isActive = step.id === currentStep;
        const isPast = idx < currentIndex;

        return (
          <div key={step.id} className="flex items-center gap-2.5 sm:gap-4 md:gap-6 lg:gap-8 shrink-0">
            <button
              type="button"
              onClick={() => onSelectStep && onSelectStep(step.id)}
              disabled={idx > currentIndex + 1}
              className={`group flex items-center gap-1.5 md:gap-2 text-xs md:text-sm font-bold tracking-wider uppercase transition-all duration-200 whitespace-nowrap ${
                isActive
            ...
            >
              <span
                className={`flex items-center justify-center text-xs font-mono font-bold transition-colors ${
                  isActive
                    ? 'text-[#f30d29]'
                    : isPast
